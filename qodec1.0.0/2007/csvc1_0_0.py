...
root_path=ff_path
rootdir = ff_path[0:24]

print('Root Folder: '+rootdir)#'C:\pfiles\2007\features'
for subdir, dirs, files in os.walk(rootdir):
     for file in files:
          output_str=''#reset the holder of output to file. Each iteration produces output for it's own file
          root_path=os.path.join(subdir, file)#C:\pfiles\2007\features\D0745J\filename.txt
          in_file=root_path[31:37]
          in_folder=root_path[24:30]
          #print(root_path)
 
          of_path='C:\\pfiles\\2007\\features_csv\\'+in_folder+'\\'+in_file+'.csv' #set the output path
